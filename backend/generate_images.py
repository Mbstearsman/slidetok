...
def generate_slideshow(
    ...
    if str(csv_path).lower().endswith(".ods"):
    ...
    rows = min(int(num_posts), len(df))
    bg_i = 0
    made = 0

    for r in range(rows):
        post_dir = os.path.join(output_folder, f"post_{r + 1:02d}")
        ...
        for c in range(df.shape[1]):
            ...
            base = _cover(Image.open(backgrounds[bg_i % len(backgrounds)]))
            bg_i += 1
    ...
    print(f"Done. Generated {made} slides across {rows} post(s) in {output_folder}")
